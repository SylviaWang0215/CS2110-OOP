/* NetId(s): sw883. Time spent: 2 hours, 00 minutes.
...
public class BugTree {
    ...
    public BugTree add(Human p, Human c) throws IllegalArgumentException {
    	...
    	if (p == null || c == null)
    		throw new IllegalArgumentException("cannot add c to this BugTree");
    	else if (this.contains(c))
    		throw new IllegalArgumentException("cannot add c to this BugTree");
    	else if (!this.contains(p))
    		throw new IllegalArgumentException("cannot add c to this BugTree");
    	
    	BugTree temp_tree = this.getTree(p);
    	BugTree new_tree = new BugTree(c);
    	temp_tree.children.add(new_tree);
    	return new_tree;
    }
